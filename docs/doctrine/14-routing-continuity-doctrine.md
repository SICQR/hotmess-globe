...
- `docs/doctrine/15-care-language-doctrine.md` — every route copy line, mode subtitle, and arrival prompt passes through the smoking-area test. §3 mode subtitles ("Quiet, simple, present" / "You have somewhere to be" / "Safer late-night path") are the canonical examples of D15 voice in routing. The §10 forbidden moves below are subordinate to D15's forbidden vocabulary.
