...
This is the **what-exists index**. `GLOBE_DOC_TRAIN.md` is the reading-order; this is the status map. Full per-doc evidence is in the reality audit (§1.5).

**Status legend**
- `live·partial` — code implementing this exists but is incomplete
- `MISSING` — described system does not exist in code yet (the build menu)
- `restate` — documents existing architecture, no new requirements
...
| `GLOBE_WEATHER_TIME_AND_ENVIRONMENTAL_RENDERING` | weather, day/night, environmental mood | **MISSING** |
| `GLOBE_AUDIO_REACTIVITY_AND_SOUNDSPACE_SYSTEM` | spatial audio, district ambience, beacon sound | **MISSING** |
| `GLOBE_EMOTIONAL_RENDERING_AND_NIGHTLIFE_PSYCHOLOGY` | emotional pacing / sensory atmosphere philosophy | **MISSING** |

## Map / local detail (the blurry-zoom cluster)
| Doc | Purpose | Status |
|---|---|---|
| `GLOBE_MAPBOX_LOCAL_MODE` | globe → real local street-map handoff (the real blur fix) | **MISSING** |
| `GLOBE_MAPBOX_LAYER_STACK` | local-mode layer order, privacy, perf contract | **MISSING** |
| `GLOBE_GLOBE_TO_LOCAL_TRANSITION_ANIMATION_SYSTEM` | planetary → district → street transition | **MISSING** |
...
| `GLOBE_BEACON_MONETISATION_AND_BOOST_POLICY` | boosts / paid visibility | **differs** (doc: Discovery/District/Momentum ≠ live Glow/Bump/Blast/…) |
| `GLOBE_BEACON_REPUTATION_AND_SPAM_CONTROL` | reputation, trust-weighting, abuse throttle | **MISSING** |
...
| `GLOBE_COMPONENT_CONTRACTS` | architectural boundaries for the renderers | restate |
...
| `GLOBE_DISTRICT_EDITORIAL_AND_CURATION_SYSTEM` | district editorial / human curation CMS | **MISSING** |
| `GLOBE_EVENT_ARCHIVE_AND_CULTURAL_MEMORY_SYSTEM` | nightlife memory / event archive | **MISSING** |
...
| `GLOBE_PARTNER_SUBSCRIPTION_TIERS` | partner subscription tiers | **contradicts** (clashes with live `membership_tiers` + `venue_beacon_tiers` → founding thread) |
...
| Doc | Purpose | Status |
|---|---|---|
| `GLOBE_SUPABASE_SCHEMA_MAP` | globe-relevant Supabase tables/views/RPCs/buckets | restate |
| `GLOBE_DOC_TRAIN` | reading order, ownership, build sequence (the index) | restate |
| `GLOBE_IMPLEMENTATION_PLAN` | executable build sequence | restate |

---

## Build menu (the 9 MISSING, highest-leverage first)
1. `GLOBE_MAPBOX_LOCAL_MODE` — fixes the blurry deep-zoom (foundational)
2. `GLOBE_MAPBOX_LAYER_STACK` — depends on #1
3. `GLOBE_GLOBE_TO_LOCAL_TRANSITION_ANIMATION_SYSTEM` — depends on #1/#2
4. `GLOBE_BEACON_REPUTATION_AND_SPAM_CONTROL` — trust foundation for open beacon creation
5. `GLOBE_DISTRICT_EDITORIAL_AND_CURATION_SYSTEM`
6. `GLOBE_EVENT_ARCHIVE_AND_CULTURAL_MEMORY_SYSTEM`
7. `GLOBE_WEATHER_TIME_AND_ENVIRONMENTAL_RENDERING`
8. `GLOBE_AUDIO_REACTIVITY_AND_SOUNDSPACE_SYSTEM`
9. `GLOBE_EMOTIONAL_RENDERING_AND_NIGHTLIFE_PSYCHOLOGY`

See `docs/SPRINT_PLAN_2026_05_23.md` for the sequenced build plan.
