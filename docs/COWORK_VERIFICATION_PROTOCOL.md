...
- **Throttled-mobile performance verification — OPEN GAP.** Until a CDP/DevTools-throttle-capable tool exists, the real-device pass is owned by Phil: open `hotmessldn.com/pulse?localmode=1` on an iPhone over cellular / slow-4G and confirm 5 consecutive deep-zoom → street-map handoffs render within ~10s each. **Health-check stub:** the daily production health check should flag local-mode as "✅ functionally verified (desktop) · ⏳ awaiting real-device throttled-mobile confirmation" until Phil runs the device pass, then clear the flag.
