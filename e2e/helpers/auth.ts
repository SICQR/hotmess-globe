...
export async function loginAs(
  ...
  // Open it by clicking the "I'm already filthy" CTA.
  const signinCta = page.locator("button:has-text(\"I'm already filthy\"), button:has-text('Sign in'), button:has-text('Login')").first();
  const ctaVisible = await signinCta.isVisible({ timeout: 5_000 }).catch(() => false);
  if (ctaVisible) {
