...
const roundTo4 = (n: number) => Math.round(n * 1e4) / 1e4;

export const makeTravelTimeKey = (viewer: LatLng, destination: LatLng) => {
  const vLat = roundTo4(viewer.lat);
  const vLng = roundTo4(viewer.lng);
  const dLat = roundTo4(destination.lat);
  const dLng = roundTo4(destination.lng);
...
export async function fetchTravelTime({
  ...
  const key = makeTravelTimeKey(viewer, destination);
  const now = Date.now();
  ...
  const promise = (async () => {
    try {
      const res = await fetch('/api/travel-time', {
        ...
        signal,
        body: JSON.stringify({
          origin: { lat: viewer.lat, lng: viewer.lng },
