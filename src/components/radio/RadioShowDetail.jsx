...
import { schedule, getNextEpisode, formatSchedule, generateICS, downloadICS } from './radioUtils';
import { format } from 'date-fns';
import { useRadio } from '../shell/RadioContext';

/**
 * Color themes per show slug
 */
const SHOW_THEMES = {
  'wake-the-mess': {
    primary: '#E62020',
    gradient: 'from-[#E62020]/20 to-[#B026FF]/20',
    buttonTextClass: 'text-white hover:text-black',
    careColor: '#00D9FF', // Different for care note
  },
  'dial-a-daddy': {
    primary: '#00D9FF',
    gradient: 'from-[#00D9FF]/20 to-[#39FF14]/20',
    buttonTextClass: 'text-black hover:text-black',
    careColor: '#00D9FF',
  },
  'hand-n-hand': {
    primary: '#FFEB3B',
    gradient: 'from-[#FFEB3B]/20 to-[#FF6B35]/20',
    buttonTextClass: 'text-black hover:text-black',
    careColor: '#FFEB3B',
  },
};
...
/**
 * Generic Radio Show Detail component
 * @param {Object} props
 * @param {number} [props.showIndex] - Index in schedule.shows array (legacy)
 * @param {string} [props.slug] - Show slug (preferred)
 */
export default function RadioShowDetail({ showIndex, slug }) {
  const { openRadio } = useRadio();
  
  // Support both slug (preferred) and showIndex (legacy)
  const show = slug
    ? schedule.shows.find(s => s.slug === slug)
    : schedule.shows[showIndex];
  
  if (!show) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-black uppercase mb-4">Show Not Found</h1>
          <Link to={createPageUrl('RadioSchedule')} className="text-[#E62020] hover:underline">
            ← Back to Shows
          </Link>
        </div>
      </div>
  ...
  // Get theme or fallback to first show's theme
  const theme = SHOW_THEMES[show.slug] || SHOW_THEMES['wake-the-mess'];
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-20">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <RadioIcon className="w-8 h-8" style={{ color: theme.primary }} />
              <Link
            ...
            <div className="flex flex-wrap gap-4">
              <Button
                onClick={openRadio}
                className={`hover:bg-white ${theme.buttonTextClass} font-black px-8 py-6 border-2 border-white uppercase`}
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 space-y-12">
        ...
        <motion.div
          ...
          className="bg-black border-2 p-6 space-y-4"
          style={{ borderColor: theme.primary }}
        >
          <h2 className="text-xl font-black uppercase" style={{ color: theme.primary }}>Stingers & VO</h2>
          
          <div className="space-y-2">
            {show.stingers.map((stinger, idx) => (
              <p 
                key={idx} 
                className="text-white/90 pl-4 border-l-2"
                style={{ borderColor: theme.primary }}
              >
                {stinger}
              </p>
        ...
        <motion.div
          ...
          className="bg-black border-2 p-6"
          style={{ borderColor: theme.careColor }}
        >
          <div className="flex items-start gap-3">
            <Heart className="w-5 h-5 flex-shrink-0 mt-1" style={{ color: theme.careColor }} />
            <p className="text-white/90">
