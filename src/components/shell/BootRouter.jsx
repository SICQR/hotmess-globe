...
export default function BootRouter({ children }) {
  ...
  // D59 S2 — Trusted-contact acceptance lives ENTIRELY on PublicShell.
  // The recipient is anonymous (Safety Constitution: account-free
  // acceptance invariant). Even if the recipient is already logged in
  // as a HOTMESS user in this browser, the landing must render outside
  // OS chrome — they're acting in their role as someone else's contact,
  // not as themselves. Short-circuit before any auth gating.
  if (location.pathname.startsWith('/contact/accept/')) {
    return <PublicShell />;
  }
